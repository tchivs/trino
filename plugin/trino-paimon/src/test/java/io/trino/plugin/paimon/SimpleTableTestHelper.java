...
public class SimpleTableTestHelper
{
    ...
    public SimpleTableTestHelper(Path path, RowType rowType)
            throws Exception
    {
        new SchemaManager(LocalFileIO.create(), path).createTable(new Schema(rowType.getFields(),
                Collections.emptyList(), Collections.singletonList("a"), Collections.singletonMap("bucket", "1"), ""));
