        ...
        FileIO
{
            ...
            throws IOException
    {
        ...
        // On object stores like S3, empty directories don't actually exist
        // Create a marker file to make the directory visible
        if (objectStore) {
            Location markerLocation = location.appendPath(".trino-directory");
            try (OutputStream out = trinoFileSystem.newOutputFile(markerLocation).create()) {
    ...
    @Override
    public boolean tryToWriteAtomic(Path path, String content)
            throws IOException
    {
        // S3 and other object stores don't support rename operations
        // Use direct overwrite instead of temp file + rename approach
        if (objectStore) {
            overwriteFileUtf8(path, content);
            return true;
        }
        else {
            // For regular filesystems, use the default atomic write with rename
            return FileIO.super.tryToWriteAtomic(path, content);
        }
    }

    @Override
    public void overwriteFileUtf8(Path path, String content)
            throws IOException
    {
        // For object stores, we directly overwrite the file
        // This is safe because S3's PUT operation is atomic
        FileIO.super.overwriteFileUtf8(path, content);
    }

    @Override
    public void overwriteHintFile(Path path, String content)
            throws IOException
    {
        // Hint files can tolerate temporary non-existence
        // For object stores, direct overwrite is the only option
        overwriteFileUtf8(path, content);
    }
}
